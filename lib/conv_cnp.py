...
class ConvCNP(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_length_scale = length_scale_multiplier / self.points_per_unit
        ...
        self.final_layer = nn.Sequential(
            nn.Linear(self.conv_net.out_channels, out_channels),
        )
        init_sequential_weights(self.final_layer)
    ...
    def forward(self, x, y):
        ...
        x_min = min(torch.min(x).cpu().numpy(), self.min_x) - self.margin
        x_max = max(torch.max(x).cpu().numpy(), self.max_x) + self.margin
