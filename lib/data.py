...
__all__ = ['GPGenerator', 'SawtoothGenerator', 'PhysioNetGenerator']
...
class DataGenerator(metaclass=abc.ABCMeta):
    ...
    def generate_task(self):
        ...
        if self.plot:
            x = np.linspace(*self.x_range,
                            num=self.num_points_per_batch).astype(np.float32)
        else:
            while True:
                x = np.sort(_rand(self.x_range, 
                    self.num_points_per_batch)).astype(np.float32)
                if (x[1:] - x[:-1] > 0).all():
                    break
        ...
        if self.extrapolation:
            if self.plot:
                x_extrap = np.linspace(*self.extrapolation_range, 
                    num=self.num_points_per_batch//2)[1:].astype(np.float32)              
            else:
                num_test_points = np.random.randint(3, self.max_test_points + 1)
                while True:
                    x_extrap = np.sort(_rand(self.extrapolation_range, 
                        num_test_points)).astype(np.float32)
                    if (x_extrap[1:] - x_extrap[:-1] > 0).all():
                        break
            x = np.concatenate([x, x_extrap])

        task = {'mask_obs': np.zeros((self.batch_size, len(x), 1)),
                'mask_y': np.zeros((self.batch_size, len(x), 1)),
                'y': np.zeros((self.batch_size, len(x), 1))}

        num_train_points = np.random.randint(3, self.max_train_points + 1)

        x_context = np.zeros((self.batch_size, num_train_points, 1))
        y_context = np.zeros((self.batch_size, num_train_points, 1))

        for i in range(self.batch_size):
            if self.plot:
                num_points = self.num_points_per_batch
            else:
                num_test_points = np.random.randint(3, self.max_test_points + 1)
                num_points = num_train_points + num_test_points

            points_indicies = np.random.choice(np.arange(self.num_points_per_batch),
                                               size=num_points,
                                               replace=False)

            points_indicies = np.concatenate([points_indicies,
                                              np.arange(self.num_points_per_batch, len(x))])

            task['y'][i, points_indicies, 0] = self.sample(x[points_indicies].astype(np.float64))

            task['mask_obs'][i, points_indicies[:num_train_points]] = 1
            task['mask_y'][i, points_indicies] = 1

            x_context[i, :, 0] = x[points_indicies[:num_train_points]]
            y_context[i] = task['y'][i, points_indicies[:num_train_points]]
        ...
        task = {k: torch.tensor(v[:, used_time_steps], 
                dtype=torch.float32).to(device) for k, v in task.items()}

        task['x'] = torch.tensor(x[used_time_steps], dtype=torch.float32).to(device)
        task['extrap_time'] = self.x_range[1]
        task['extrap_index'] = (x[used_time_steps] <= self.x_range[1]).sum()
        task['x_context'] = torch.tensor(x_context, dtype=torch.float32).to(device)
        task['y_context'] = torch.tensor(y_context, dtype=torch.float32).to(device)
